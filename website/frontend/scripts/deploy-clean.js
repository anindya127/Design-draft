    ...
    try {
        ...
        for (const item of list) {
            ...
            if (item.type === 'd') {
                ...
                await deleteRemoteDir(sftp, fullPath);
        ...
        if (err.code !== 2) { // Ignore "no such file" errors
            throw err;
        }
    }
}
...
async function deploy() {
    ...
    try {
        ...
        console.log('🗑️  Deleting all existing files from server...');
        await deleteRemoteDir(sftp, REMOTE_DIR);
        ...
        console.log(`\n\n✅ Clean deploy complete! ${files.length} files uploaded in ${formatTime(elapsed)}`);
